...
int main(int argc , char *argv[])	{  
	...
	while(TRUE) {  
		...
		if (flag <= 0) {
			...
			if (server_socket <= 0) {
				...
				if (connect(server_socket, (struct sockaddr*)&servaddr, sizeof(servaddr)) != 0) {
					...
					// Send heart beat
					sendHeartbeat(curr_session_id, curr_seqN, curr_ackN, server_socket);
		...
		ti = (curr_time.tv_sec - last_hb_recv.tv_sec);
		if (ti >= 3) {
			printf("Disconnecting from sproxy...\n");
			close(server_socket);
			server_socket = 0;
			flag = 0;
		}
		...
		if (FD_ISSET(client_socket , &readfds)) { 
			...
			//incoming message 
			if ((valread = read( client_socket , buffer, 1025)) == 0) {  
			...
			//Echo the message to the server
				...
				send(server_socket, headerToSend, 20, 0);
		...
		if (FD_ISSET( server_socket , &readfds))  {
			//Check if it was for closing , and also read the 
			//incoming message 
			if ((valread = read( server_socket , header, 20)) <= 0)  {  
				//Somebody disconnected , get his details and print 
				...
				if (isHeartbeat) {
					...
					valread = read(server_socket, buffer, length);
				...
				resendPackets(head, server_socket);
